...
- **Vị trí nhập bằng địa chỉ + ghép ghim trên bản đồ, không bắt người dùng tự gõ lat/lng.** Form địa chỉ theo địa giới 2 cấp (từ 1/7/2025): số nhà + đường / phường-xã / tỉnh-thành phố (chỉ để hiển thị) + bản đồ Leaflet/OSM để chọn toạ độ chính xác. Không dùng API geocoding bên thứ 3 (Goong cần admin duyệt key; Nominatim chặn IP server) — quyết định đã chốt, đừng thêm lại geocoding server-side mà không hỏi trước.
